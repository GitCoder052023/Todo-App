...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (loginResult.success) {
            ...
            res.json({ message: "Login successful", email: currentUserEmail });
        } else {
...
app.get('/api/todos', async (req, res) => {
  ...
  app.post('/api/todos', async (req, res) => {
    try {
      ...
      const result = await db.collection('todos').insertOne({ text, completed: false, email });
      res.status(201).json({ _id: result.insertedId, text, completed: false, email });
    } catch (error) {
